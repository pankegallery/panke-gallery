...
### 3.7 Persistent language indicator in the guide header — built

Original suggestion: shrink the logo in `GuideLayout`'s header and show the current language there instead, clickable to reopen the picker, from any guide page.

- [x] `GuideLayout` stays data-agnostic — it takes a `headerAction` node prop rather than knowing about languages directly. The header's right-hand slot (`HeaderActions`, [GuideLayout.styles.js](src/components/guide-layout/GuideLayout.styles.js)) renders `headerAction` **and** the "back to overview" icon together, side by side, not one-or-the-other — a stop page needs both at once (see 3.8 below for why).
- [x] Exhibition overview ([guide-exhibition.js](src/templates/guide-exhibition.js)) and the global `/guide/` overview ([guide.js](src/pages/guide.js)) have no overview icon (`showOverviewLink={false}`), so their header shows just the language action.
- [x] On the exhibition overview, the label is clickable (`LanguageLabel` with `onClick`) and clears the in-memory selection to re-show the picker. On the global overview, it's the read-only variant (no `onClick`) — per the original evaluation, there's no single exhibition's language list to switch within from that page.
- [x] `LanguageLabel` ([src/components/language-label.js](src/components/language-label.js)) is styled as a bordered pill with an icon (`faLanguage`), matching the transcript "Read/Hide" toggle's look, not plain text — both the clickable and read-only variants share the same box model (padding/border) so the header never changes height depending on which is shown (see the fix below).
- [x] Logo shrunk (`SmallLogotype`, `theme.fontSizes.medium` instead of `large`) and now opens `panke.gallery` in a new tab (`target="_blank" rel="noopener noreferrer"`) — an accidental tap no longer navigates the visitor away from the guide.
